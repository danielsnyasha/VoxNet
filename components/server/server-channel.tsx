...
const ServerChannel = ({
    ...
    return (
        <button onClick={onClick} className={cn('group px-2 py-2 rounded-md flex items-center gap-x-2 w-full hover:bg-zinc-700/10 dark:hover:bg-zinc-700/50 transition mb-1', params?.channelId === channel.id && 'bg-zinc-700/20')}>
            ...
            {channel.name !== "general" && role !== MemberRole.GUEST && (
                <div className='ml-auto flex items-center gap-x-2'>
                    ...
                    <ActionTooltip label='Delete'>
                        <Trash onClick={(e) => onAction(e, "deleteChannel")} className='hidden group-hover:block w-4 h-4 text-zinc-500 hover:text-red-100 dark:text-zinc-400 dark:hover:text-zinc-300 transition' />
                    </ActionTooltip>
                </div>
            )}
            {channel.name === "general" && (
                <Lock className='hover:text-red-200 ml-auto w-4 h-4 text-zinc-500 dark:text-zinc-500' />
            )}
        </button>
    );
};
